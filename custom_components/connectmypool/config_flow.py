    ...
    async def async_step_user(self, user_input=None) -> FlowResult:
        ...
        if user_input is not None:
            ...
            try:
                ...
                await api.pool_config(pool_api_code)
                ...
                await self.async_set_unique_id(pool_api_code)
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Optional(
                        CONF_TEMPERATURE_SCALE,
                        default=self.config_entry.options.get(CONF_TEMPERATURE_SCALE, self.config_entry.data.get(CONF_TEMPERATURE_SCALE, DEFAULT_TEMPERATURE_SCALE)),
                    ): vol.In([0, 1]),
                    vol.Optional(
                        CONF_BASE_URL,
                        default=self.config_entry.options.get(CONF_BASE_URL, self.config_entry.data.get(CONF_BASE_URL, DEFAULT_BASE_URL)),
                    ): str,
                }
            ),
        )
